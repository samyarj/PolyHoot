...
describe('TimeService', () => {
    ...
    it('resetTimer should call stopTimer', () => {
        const spy = spyOn(service, 'stopTimer').and.stub();
        const testTimeValue = 5;
        service.resetTimer(testTimeValue);
        expect(spy).toHaveBeenCalled();
    });

    it('resetTimer should call startTimer with the good parameter', () => {
        const spy = spyOn(service, 'startTimer').and.stub();
        const testTimeValue = 5;
        service.resetTimer(testTimeValue);
        expect(spy).toHaveBeenCalledWith(testTimeValue);
    });
});
