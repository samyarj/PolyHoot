...
export class AuthService {
    ...
    restoreSession(): void {
        const token = localStorage.getItem('firebaseToken');
        if (token) {
            this.http.get<User>(`${this.baseUrl}/profile`, { headers: { Authorization: `Bearer ${token}` } }).subscribe({
    ...
    checkingUsername(username: string): Observable<boolean> {
        return this.http
            .get<{ usernameExists: boolean }>(`${this.baseUrl}/check-username`, { params: { username } })
            .pipe(map((response) => response.usernameExists));
    }

    checkingEmail(email: string): Observable<boolean> {
        return this.http
            .get<{ emailExists: boolean }>(`${this.baseUrl}/check-email`, { params: { email } })
